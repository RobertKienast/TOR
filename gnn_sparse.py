"""
Sparse-graph GNN meta-optimizer.

This module builds a sparse parameter graph (top-k similarity neighbors) and
runs message passing only on those edges to reduce compute and focus updates on
strongly related parameter nodes.
"""
...
def build_sparse_topk_edges(
    ...
    then connect each anchor to its top-k most-similar NEARBY node (index-
    window proximity, not a random candidate drawn from the whole graph).

    Args:
        node_feats: (N, D)
        k: neighbors kept per anchor node
        min_similarity: optional pruning threshold
    ...
    z = F.normalize(node_feats, dim=-1)
    scores = (z[anchors].unsqueeze(1) * z[dst]).sum(dim=-1)  # (num_anchors, cand_per_anchor)
...
class GNNSparseMetaLearner(nn.Module):
    """GNN optimizer that constructs and optimizes over a sparse graph."""
    ...
    def forward(
        ...
        Args:
            ...
            edge_index: optional external edges. If None, sparse edges are built.
        ...
        if sparse_edge_index is None:
            sparse_edge_index = build_sparse_topk_edges(
                h,
                k=self.sparse_k,
...
__all__ = ["GNNSparseMetaLearner", "build_sparse_topk_edges"]


def _build_model_from_args(args, config: Optional[dict] = None) -> GNNSparseMetaLearner:
    config = config or {}
    return GNNSparseMetaLearner(
        ...
        min_similarity=float(config.get("min_similarity", args.min_similarity)),
...
def _save_checkpoint(model: GNNSparseMetaLearner, save_path: str) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train or evaluate the cosine-sparse GNN meta-optimizer")
    ...
    train.add_argument("--min_similarity", type=float, default=-0.25)
    ...
    train.add_argument("--save", type=str, default="gnn_sparse.pt")
    ...
    eval_parser.add_argument("--min_similarity", type=float, default=-0.25)
    ...
    if args.cmd == "eval":
        ...
        print(f"gnn_sparse mean loss: {avg_loss:.6f}")
