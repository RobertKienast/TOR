...
EPSILON = 1e-6
# NumPy on some platforms uses int32 bounds for randint high.
...
class Rastrigin(Problem):
  ...
  def objective(self, x, data=None, labels=None):
    ...
    ras_norm=tf.norm(pre_norm,ord=2,axis=-1)
    cqTcos=tf.squeeze(tf.matmul(tf.transpose(self.c,perm=[1, 0]), tf.cos(2 * np.pi * x[0])))
