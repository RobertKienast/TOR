...
def _maybe_download_cifar10(path):
  ...
  if not os.path.exists(filepath):
    ...
    url = "{}/{}".format(CIFAR10_URL.rstrip("/"), CIFAR10_FILE)
...
def LeNet(path,
    ...
    if batch_norm:
      linear_activation = lambda x: tf.sigmoid(snt.BatchNorm()(x, is_training=is_training))
    else:
    ...
    def build():
        ...
        if batch_norm:
            conv_out = conv(image_batch, is_training=is_training)
        else:
            conv_out = conv(image_batch)

        output = mlp(snt.BatchFlatten()(conv_out))
        return _xent_loss(output, label_batch)
