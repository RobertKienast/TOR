...
#        pdb.set_trace()
        ...
        # Convert any namedtuples (e.g. LSTMState) to plain tuples so that
        # tf.while_loop sees the same nested structure on every iteration.
        def _to_plain_tuple(s):
          if hasattr(s, '_fields'):
            return tuple(_to_plain_tuple(v) for v in s)
          elif isinstance(s, (list, tuple)):
            return type(s)(_to_plain_tuple(v) for v in s)
          return s
        state_next = [_to_plain_tuple(s) for s in state_next]
